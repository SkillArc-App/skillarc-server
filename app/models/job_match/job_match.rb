    ...
    def jobs
      save_events = Event
                    .where(
                      ...
                      event_type: [Messages::Types::User::JOB_SAVED, Messages::Types::User::JOB_UNSAVED]
                    )
                    .map(&:message)
      ...
      @jobs ||= Job.shown.with_everything.map do |job|
...

        {
          ...
          saved: save_events[job.id]&.sort_by(&:occurred_at)&.last&.schema&.message_type == Messages::Types::User::JOB_SAVED,
