class MessageService
  ...
  def create_once_for_trace!(schema:, data:, stream: nil, trace_id: SecureRandom.uuid, id: SecureRandom.uuid, occurred_at: Time.zone.now, metadata: Messages::Nothing, **) # rubocop:disable Metrics/ParameterLists
    aggregate = get_aggregate(stream:, schema:, **)
  ...
  def create_once_for_aggregate!(schema:, data:, stream: nil, trace_id: SecureRandom.uuid, id: SecureRandom.uuid, occurred_at: Time.zone.now, metadata: Messages::Nothing, **) # rubocop:disable Metrics/ParameterLists
    aggregate = get_aggregate(stream:, schema:, **)
    message = build(schema:, data:, trace_id:, id:, occurred_at:, metadata:, stream:, **)

    projection = Projectors::Aggregates::HasOccurred.project(stream:, schema:)
    raise MessageService::NotBooleanProjection unless [true, false].include?(projection)
  ...
  def create!(schema:, data:, stream: nil, trace_id: SecureRandom.uuid, id: SecureRandom.uuid, occurred_at: Time.zone.now, metadata: Messages::Nothing, **) # rubocop:disable Metrics/ParameterLists
    message = build(schema:, data:, trace_id:, id:, occurred_at:, metadata:, stream:, **)
    save!(message)
  ...
  def build(schema:, data:, stream: nil, trace_id: SecureRandom.uuid, id: SecureRandom.uuid, occurred_at: Time.zone.now, metadata: Messages::Nothing, **) # rubocop:disable Metrics/ParameterLists
  ...
  def self.stream_events(aggregate)
    raise NotAggregateError unless aggregate.is_a?(Messages::Stream)
  ...
  def versioned_type(message_type, version)
    "#{message_type} #{version}"
  end

  def get_aggregate(stream:, schema:, **)
    aggregate || schema.stream.new(**)
  end
