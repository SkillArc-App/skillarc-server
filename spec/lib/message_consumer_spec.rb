  ...
  describe ".on_message" do
    let(:sub_klass) do
      Class.new(described_class) do
        on_message Users::Events::SessionStarted::V1, &:serialize
      end
    end

    let(:message) { build(:message, schema: Users::Events::SessionStarted::V1, data: Core::Nothing) }
    ...
    context "when on_message is sync" do
      let(:sub_klass) do
        Class.new(described_class) do
          on_message Users::Events::SessionStarted::V1, :sync, &:checksum
        end
      end
      ...
      it "correctly reports how it handles events" do
        ...
        expect(consumer.all_handled_messages).to eq([Users::Events::SessionStarted::V1])
        expect(consumer.handled_messages_sync).to eq([Users::Events::SessionStarted::V1])
    ...
    context "when on_message is async" do
      let(:sub_klass) do
        Class.new(described_class) do
          on_message Users::Events::SessionStarted::V1, &:checksum
        end
      end
      ...
      it "correctly reports how it handles events" do
        ...
        expect(consumer.handled_messages).to eq([Users::Events::SessionStarted::V1])
      end
    end
  end
