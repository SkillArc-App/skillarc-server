...
RSpec.describe JobMatch::JobMatch do
  ...
  before do
    create(:event, :job_saved, aggregate_id: user.id, data: Events::JobSaved::Data::V1.new(job_id: saved_job.id, employment_title: "A", employer_name: "B").to_h)
    create(:event, :job_unsaved, aggregate_id: user.id, data: Events::JobUnsaved::Data::V1.new(job_id: saved_job.id, employment_title: "A", employer_name: "B").to_h)
    create(:event, :job_saved, aggregate_id: user.id, data: Events::JobSaved::Data::V1.new(job_id: saved_job.id, employment_title: "A", employer_name: "B").to_h)
  end
