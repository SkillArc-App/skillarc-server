module Klaviyo
  class KlaviyoReactor < MessageReactor # rubocop:disable Metrics/ClassLength
    ...
    on_message Events::PersonAdded::V1 do |message|
      email = email_for_person_aggregate(message.stream)
    ...
    on_message Events::JobSaved::V1 do |message|
      dedup_messages(message) do
        client.job_saved(
          email: email_for_user_aggregate(message.stream),
          event_id: message.id,
          event_properties: {
    ...
    def dedup_messages(message)
      ...
      return if Projectors::Aggregates::HasOccurred.project(stream:, schema: Events::KlaviyoEventPushed::V1)
    ...
    def email_for_user_aggregate(_aggregate)
